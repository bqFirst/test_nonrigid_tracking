...
EIGEN_DEFINE_STL_VECTOR_SPECIALIZATION(n_i);
...
bool
test_non_rigid_track(Eigen::Vector3d* vertex, const Eigen::Matrix4d &cameraMatrix, Eigen::Vector3d* inputVertex, const Eigen::Matrix4d &pose) {
	Eigen::Matrix4d rigid_t = pose;
	double avg_diff_norm = 0.0;
	//CVD::Image<u_int16_t>depthmap_n(CVD::ImageRef(width, height));
	cv::Mat depthmap_n(height,width,CV_16U);
	depthmap_n.convertTo(depthmap_n, CV_32F);
	int unacceptable_n = 0;
	int pixelx, pixely;
	for (pixely = 0; pixely < height; pixely++) {
		for (pixelx = 0; pixelx < width; pixelx++) {
			//depthmap_n[CVD::ImageRef(pixelx,pixely)] = (u_int16_t)0;
			depthmap_n.at<float>(pixely, pixelx) =  0;
			...
			depthmap_n.at <float>(pixelx, pixely) = norm * 5000;
			avg_diff_norm += norm;
		}
	}
	...
	//cvSaveImage(fileName,depthmap_n);
	cv::imwrite(fileName,depthmap_n);
...
int main()
{
	...
	Eigen::Vector3d refvertex[240 * 320];
	Eigen::Vector3d invertex[240 * 320];
	Eigen::Vector3d normal[240 * 320];
	...
	for (int i = 0; i < 50; i++) {
		...
		cv::Mat DEPTHPNG = cv::imread (fileNamepng,CV_LOAD_IMAGE_ANYDEPTH);
		DEPTHPNG.convertTo(DEPTHPNG, CV_32F); // convert the image data to float type
		...
		for (int v = 0; v < height; v++)
		{
			for (int u = 0; u < width; u++)
			{
				...
				bool inrange = DEPTHPNG.at<float>(v, u) < 8000 && u > 21 && u < 200; 
				//bool inrange = true;
				depthMap[u + v * width] = inrange ? (DEPTHPNG.at<float>(v, u)) / 5000.0f : 0;
